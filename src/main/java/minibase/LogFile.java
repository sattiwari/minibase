...
/**
 LogFile implements the recovery subsystem of minibase.  This class is
...
public class LogFile {
     ...
     * We're not sure yet whether the caller is creating a brand new DB, in which
     * case we should ignore the log file, or whether the caller will eventually
     * want to recover (after populating the Catalog). So we make this decision
     * lazily: if someone calls recover(), then do it, while if someone starts
     * adding log file entries, then first throw out the initial log file
     * contents.
     *
    ...
    public synchronized void logXactionBegin(TransactionId tid) throws IOException {
        Debug.log("BEGIN");
        if (tidToFirstLogRecord.get(new Long(tid.getId())) != null) {
            ...
            try {
                ...
                switch (type) {
                    ...
                    case CHECKPOINT_RECORD:
                        ...
                        break;
                    case BEGIN_RECORD:
                        tidToFirstLogRecord.put(new Long(record_tid), new Long(newStart));
                        break;
                }
     ...
     * should not be called on transactions that have already committed (though
