...
public class Parser {
    ...
    static Predicate.Op getOp(String s) throws minibase.ParsingException {
        ...
        throw new minibase.ParsingException("Unknown predicate " + s);
    }

    static void processExpression(TransactionId tid, ZExpression wx, LogicalPlan lp) throws minibase.ParsingException {
        if (wx.getOperator().equals("AND")) {
            for (int i = 0; i < wx.nbOperands(); i++) {
                if (!(wx.getOperand(i) instanceof ZExpression)) {
                    throw new minibase.ParsingException("Nested queries are currently unsupported.");
                }
            ...
            throw new minibase.ParsingException("OR expressions currently unsupported.");
        } else {
            // this is a binary expression comparing two constants
            @SuppressWarnings("unchecked")
            Vector<ZExp> ops = wx.getOperands();
            if (ops.size() != 2) {
                throw new minibase.ParsingException("Only simple binary expresssions of the form A op B are currently supported.");
            }
            ...
            if (op1const && op2const) {
                ...
                throw new minibase.ParsingException("Only simple binary expresssions of the form A op B are currently supported, where A or B are fields, constants, or subqueries.");
            } else isJoin = false;
            ...
            if (isJoin) {       //join node
                ...
                if (!op2const) { //right op is a nested query
                    try {
                        ...
                        throw new minibase.ParsingException("Invalid subquery " + ops.elementAt(1));
    ...
    public static LogicalPlan parseQueryLogicalPlan(TransactionId tid, ZQuery q) throws IOException, Zql.ParseException, minibase.ParsingException {
        @SuppressWarnings("unchecked")
        ...
        for (int i = 0; i < from.size(); i++) {
            ...
            try {
                ...
                e.printStackTrace();
                throw new minibase.ParsingException("Table " + fromIt.getTable() + " is not in catalog");
            }
        }
        ...
        if (gby != null) {
            ...
            if (gbs.size() > 1) {
                throw new minibase.ParsingException("At most one grouping field expression supported.");
            }
            if (gbs.size() == 1) {
                ZExp gbe = gbs.elementAt(0);
                if (! (gbe instanceof ZConstant)) {
                    throw new minibase.ParsingException("Complex grouping expressions (" + gbe + ") not supported.");
                }
        ...
        for (int i = 0; i < selectList.size(); i++) {
            ...
            if (si.getAggregate() == null && (si.isExpression() && !(si.getExpression() instanceof ZConstant))) {
                throw new minibase.ParsingException("Expressions in SELECT list are not supported.");
            }
            if (si.getAggregate() != null) {
                if (aggField != null) {
                    throw new minibase.ParsingException("Aggregates over multiple fields not supported.");
                }
                ...
                if (groupByField != null && ! (groupByField.equals(si.getTable() + "." + si.getColumn()) || groupByField.equals(si.getColumn()))) {
                    throw new minibase.ParsingException("Non-aggregate field " + si.getColumn() + " does not appear in GROUP BY list.");
                }
        ...
        if (groupByField != null && aggFun == null) {
            throw new minibase.ParsingException("GROUP BY without aggregation.");
        }
        ...
        if (q.getOrderBy() != null) {
            ...
            if (obys.size() > 1)  {
                throw new minibase.ParsingException("Multi-attribute ORDER BY is not supported.");
            }
            ZOrderBy oby = obys.elementAt(0);
            if (!(oby.getExpression() instanceof ZConstant)) {
                throw new minibase.ParsingException("Complex ORDER BY's are not supported");
            }
    ...
    public static DbIterator parseQuery(TransactionId tid, ZQuery q) throws IOException, Zql.ParseException, minibase.ParsingException {
        return parseQueryLogicalPlan(tid, q).physicalPlan(tid, statsMap, explain);
    }

    static Transaction curtrans = null;

    public static void handleQueryStatement(ZQuery s) throws TransactionAbortedException, DbException, IOException, minibase.ParsingException, Zql.ParseException  {
        // and run it
        DbIterator node;
    ...
    public static void handleInsertStatement(ZInsert s) throws TransactionAbortedException, DbException, IOException, minibase.ParsingException, Zql.ParseException  {
        int id;
        try {
            id = Database.getCatalog().getTableId(s.getTable()); //will fall through if table doesn't exist
        } catch (NoSuchElementException e) {
            throw new minibase.ParsingException ("Unknown table : " + s.getTable());
        }
        ...
        if (s.getValues() != null) {
            ...
            if (td.numFields() != values.size()) {
                throw new minibase.ParsingException("INSERT statement does not contain same number of fields as table " + s.getTable());
            }
            for (ZExp e : values) {

                if (!(e instanceof ZConstant))
                    throw new minibase.ParsingException("Complex expressions not allowed in INSERT statements.");
                ZConstant zc = (ZConstant)e;
                if (zc.getType() == ZConstant.NUMBER) {
                    if (td.getFieldType(i) != Type.INT_TYPE) {
                        throw new minibase.ParsingException("Value " + zc.getValue() + " is not an integer, expected a string.");
                    }
                    IntField f= new IntField(new Integer(zc.getValue()));
                    t.setField(i,f);
                } else if(zc.getType() == ZConstant.STRING) {
                    if (td.getFieldType(i) != Type.STRING_TYPE) {
                        throw new minibase.ParsingException("Value " + zc.getValue() + " is a string, expected an integer.");
                    }
                    StringField f= new StringField(zc.getValue(), Type.STRING_LEN);
                    t.setField(i,f);
                } else {
                    throw new minibase.ParsingException("Only string or int fields are supported.");
                }
    ...
    public static void handleDeleteStatement(ZDelete s) throws TransactionAbortedException, DbException, IOException, minibase.ParsingException, Zql.ParseException  {
        int id;
        try {
            id = Database.getCatalog().getTableId(s.getTable()); //will fall through if table doesn't exist
        } catch (NoSuchElementException e) {
    ...
    public static void handleTransactStatement(ZTransactStmt s) throws TransactionAbortedException, DbException, IOException, minibase.ParsingException, Zql.ParseException {
        if (s.getStmtType().equals("COMMIT")) {
            ...
            throw new minibase.ParsingException("Can't start new transactions until current transaction has been committed or rolledback.");
        }
    }

    public static LogicalPlan generateLogicalPlan(TransactionId tid, String s) throws minibase.ParsingException {
        ...
        try {
            ...
            throw new minibase.ParsingException("Invalid SQL expression: \n \t " + e);
        }
        catch (IOException e) {
            throw new minibase.ParsingException(e);
        }

        throw new minibase.ParsingException("Cannot generate logical plan for expression : " + s);
    }
    ...
    public static void processNextStatement(InputStream is) {
        try {
            ...
            e.printStackTrace();
        } catch (minibase.ParsingException e) {
    ...
    public static void main(String argv[]) throws IOException {
        ...
        if (!interactive) {
            ...
            String line;
            while ((line = reader.readLine("minibase> ")) != null) {
