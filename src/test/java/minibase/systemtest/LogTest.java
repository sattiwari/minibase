...
import minibase.DbException;
import minibase.HeapFile;
import minibase.HeapPage;
import minibase.HeapPageId;
import minibase.Insert;
import minibase.IntField;
import minibase.Page;
import minibase.Permissions;
import minibase.SeqScan;
import minibase.Transaction;
import minibase.TransactionAbortedException;
import minibase.Tuple;
import minibase.TupleDesc;
import minibase.TupleIterator;
...
public class LogTest extends SimpleDbTestBase {
    ...
    void insertRow(HeapFile hf, Transaction t, int v1, int v2) throws DbException,
        ...
        TupleIterator insertRow = new TupleIterator(Utility.getTupleDesc(2),
                Arrays.asList(new Tuple[] { value }));
    ...
    void look(HeapFile hf, Transaction t, int v1, boolean present) throws DbException,
            TransactionAbortedException {
        int count = 0;
    ...
    // insert tuples
    void doInsert(HeapFile hf, int t1, int t2) throws DbException, TransactionAbortedException,
            IOException {
    ...
    void abort(Transaction t) throws IOException {
        ...
        Database.getBufferPool().flushAllPages(); // prevent NO-STEAL-based abort
        // from
        // un-doing the rollback
        Database.getBufferPool().transactionComplete(t.getId(), false); // release
        // locks
    }
    ...
    // abort
    void dontInsert(HeapFile hf, int t1, int t2) throws DbException, TransactionAbortedException,
            IOException {
        Transaction t = new Transaction();
        t.start();
        if (t1 != -1)
    ...
    public void PatchTest() throws IOException, DbException, TransactionAbortedException {
        ...
        t1.start();
        Page p = Database.getBufferPool().getPage(t1.getId(), new HeapPageId(hf1.getId(), 0),
    ...
    @Test
    public void TestAbortCommitInterleaved() throws IOException, DbException,
            TransactionAbortedException {
        setup();
        doInsert(hf1, 1, 2);
    ...
    @Test
    public void TestAbortCrash() throws IOException, DbException, TransactionAbortedException {
        setup();
        doInsert(hf1, 1, 2);
    ...
    @Test
    public void TestCommitAbortCommitCrash() throws IOException, DbException,
            TransactionAbortedException {
        setup();
        doInsert(hf1, 1, 2);
    ...
    @Test
    public void TestOpenCommitOpenCrash() throws IOException, DbException,
            TransactionAbortedException {
        setup();
        doInsert(hf1, 1, 2);
    ...
    @Test
    public void TestOpenCommitCheckpointOpenCrash() throws IOException, DbException,
            TransactionAbortedException {
        setup();
        doInsert(hf1, 1, 2);
