...
public class SystemTestUtil {
    ...
    public static HeapFile createRandomHeapFile(int columns, int rows,
    ...
    public static File createRandomHeapFileUnopened(int columns, int rows, int maxValue,
                                                    Map<Integer, Integer> columnSpecification, ArrayList<ArrayList<Integer>> tuples)
            throws IOException {
        ArrayList<ArrayList<Integer>> generatedTuples = new ArrayList<ArrayList<Integer>>(rows);
        ...
        for (int i = 0; i < rows; ++i) {
            ...
            for (int j = 0; j < columns; ++j) {
                ...
                if (columnSpecification != null)
                    columnValue = columnSpecification.get(new Integer(j));
                if (columnValue == null) {
                    columnValue = new Integer(r.nextInt(maxValue));
                }
        ...
        temp.deleteOnExit();
        HeapFileEncoder.convert(generatedTuples, temp, BufferPool.getPageSize(), columns);

        if (tuples != null) {
            tuples.clear();
            tuples.addAll(generatedTuples);
        }
        return temp;
    }
    ...
    public static void matchTuples(DbFile f, List<ArrayList<Integer>> tuples) throws DbException,
    ...
    /**
     * Returns number of bytes of RAM used by JVM after calling System.gc many
     * times.
     *
     ...
     * ret[0] is true if the sequence is quadratic ret[1] is the common difference
     * of the sequence if ret[0] is true.
     *
     * @param sequence
     * @return ret[0] = true if sequence is qudratic(or sub-quadratic or linear),
     *         ret[1] = the coefficient of n^2
     */
    public static Object[] checkQuadratic(double[] sequence) {
        Object ret[] = checkLinear(getDiff(sequence));
        ret[1] = new Double(((Double) ret[1]).doubleValue() / 2.0);
        return ret;
    }

    /**
     * Checks if the sequence represents an arithmetic sequence (approximately)
     * ret[0] is true if the sequence is linear ret[1] is the common difference of
     * the sequence if ret[0] is true.
     *
     * @param sequence
    ...
    /**
     * Checks if the sequence represents approximately a fixed sequence
     * (c,c,c,c,..) ret[0] is true if the sequence is linear ret[1] is the
     * constant of the sequence if ret[0] is true.
     *
     * @param sequence
