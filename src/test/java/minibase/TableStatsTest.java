...
public class TableStatsTest extends SimpleDbTestBase {
    ...
    private double[] getRandomTableScanCosts(int[] pageNums, int[] ioCosts) throws IOException {
        ...
        for (int i = 0; i < ioCosts.length; ++i) {
            ...
            String tableName2 = SystemTestUtil.getUUID();
            Database.getCatalog().addTable(hf, tableName2);
            int tableId2 = Database.getCatalog().getTableId(tableName2);
            ret[i] = (new TableStats(tableId2, ioCosts[i])).estimateScanCost();
        }
        return ret;
    }

    /**
     * Verify the cost estimates of scanning various numbers of pages from a
     * HeapFile This test checks that the estimateScanCost is: +linear in numPages
     * when IO_COST is constant +linear in IO_COST when numPages is constant
     * +quadratic when IO_COST and numPages increase linearly.
     */
    @Test
    public void estimateScanCostTest() throws IOException {
        Object[] ret;
    ...
    /**
     * Verify that selectivity estimates do something reasonable. Don't bother
     * splitting this into N different functions for each possible Op because we
     * will probably catch any bugs here in IntHistogramTest, so we hopefully
     * don't need all the JUnit checkboxes.
     */
    @Test
    public void estimateSelectivityTest() {
        final int maxCellVal = 32; // Tuple values are randomized between 0 and this
        // number
        ...
        for (int col = 0; col < 10; col++) {
            ...
            Assert.assertEquals(1.0 / 32.0, s.estimateSelectivity(col, Predicate.Op.EQUALS, halfMaxMin),
                    0.015);
            Assert.assertEquals(0, s.estimateSelectivity(col, Predicate.Op.EQUALS, belowMin), 0.001);

            Assert
                    .assertEquals(1.0, s.estimateSelectivity(col, Predicate.Op.NOT_EQUALS, aboveMax), 0.001);
            Assert.assertEquals(31.0 / 32.0,
                    s.estimateSelectivity(col, Predicate.Op.NOT_EQUALS, halfMaxMin), 0.015);
            Assert
                    .assertEquals(1.0, s.estimateSelectivity(col, Predicate.Op.NOT_EQUALS, belowMin), 0.015);

            Assert.assertEquals(0.0, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN, aboveMax),
                    0.001);
            Assert.assertEquals(0.0, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN, atMax), 0.001);
            Assert.assertEquals(0.5, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN, halfMaxMin),
                    0.1);
            Assert.assertEquals(31.0 / 32.0,
                    s.estimateSelectivity(col, Predicate.Op.GREATER_THAN, atMin), 0.05);
            Assert.assertEquals(1.0, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN, belowMin),
                    0.001);
            ...
            Assert.assertEquals(0.0,
                    s.estimateSelectivity(col, Predicate.Op.GREATER_THAN_OR_EQ, aboveMax), 0.001);
            Assert.assertEquals(0.0, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN_OR_EQ, atMax),
                    0.015);
            Assert.assertEquals(0.5,
                    s.estimateSelectivity(col, Predicate.Op.GREATER_THAN_OR_EQ, halfMaxMin), 0.1);
            Assert.assertEquals(1.0, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN_OR_EQ, atMin),
                    0.015);
            Assert.assertEquals(1.0,
                    s.estimateSelectivity(col, Predicate.Op.GREATER_THAN_OR_EQ, belowMin), 0.001);

            Assert.assertEquals(1.0, s.estimateSelectivity(col, Predicate.Op.LESS_THAN_OR_EQ, aboveMax),
                    0.001);
            Assert.assertEquals(1.0, s.estimateSelectivity(col, Predicate.Op.LESS_THAN_OR_EQ, atMax),
                    0.015);
            Assert.assertEquals(0.5,
                    s.estimateSelectivity(col, Predicate.Op.LESS_THAN_OR_EQ, halfMaxMin), 0.1);
            Assert.assertEquals(0.0, s.estimateSelectivity(col, Predicate.Op.LESS_THAN_OR_EQ, atMin),
                    0.05);
            Assert.assertEquals(0.0, s.estimateSelectivity(col, Predicate.Op.LESS_THAN_OR_EQ, belowMin),
                    0.001);
        }
    }
}